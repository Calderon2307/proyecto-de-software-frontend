...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const setAuthToken = (token: string) => {
    ...
    setUser({ id: 0, nombre_entrenador: 'OAuth User', email: 'verified@google.com' });
  };
  
  const loadProfileFromToken = useCallback(() => {
    const storedToken = localStorage.getItem('jwtToken');
    if (storedToken) {
        setUser({ id: 1, nombre_entrenador: 'Trainer', email: 'default@pokedex.com' }); 
    }
  }, []);
  ...
  const login = async (email: string, password: string) => {
    ...
    const { token, id, nombre_entrenador, email: userEmail } = response.data.data;
  ...
  const register = async (data: any) => {
    ...
    const payload = {
      nombre: data.name,
      email: data.email,
      contrasenia: data.password, // Clave esperada por el backend
      region_preferida: data.preferredRegion,
      tipo_preferido: data.preferredType,
      pokemon_favorito: data.favoritePokemon, // Clave esperada por el backend
    };
  ...
  return (
    <AuthContext.Provider value={{ user, isAuthenticated, login, register, logout, setAuthToken }}>
      {children}
